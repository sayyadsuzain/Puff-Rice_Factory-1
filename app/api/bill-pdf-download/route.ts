...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (bill.party_id) {
      const { data: party } = await supabase.from('parties').select('name, gst_number').eq('id', bill.party_id).single()
    ...
    const itemRows = (items || []).map((item) => {
      ...
      return `
        <tr style="height: 32px;">
          <td style="border-left: 1px solid #9ca3af; border-right: 1px solid #e5e7eb; padding: 4px 8px; vertical-align: middle;">
          ...
          <td style="border-right: 1px solid #e5e7eb; text-align: center; padding: 4px 8px; vertical-align: middle;">${item.qty_bags || ''}</td>
          <td style="border-right: 1px solid #e5e7eb; text-align: center; padding: 4px 8px; vertical-align: middle;">${isPaddyItem ? `${item.weight_kg || ''}kg` : (item.weight_kg || '')}</td>
          <td style="border-right: 1px solid #e5e7eb; text-align: center; padding: 4px 8px; vertical-align: middle;">${item.rate ? `${item.rate.toFixed(2)}${isPaddyItem ? ' ₹/kg' : ''}` : ''}</td>
          <td style="border-right: 1px solid #9ca3af; text-align: right; font-weight: bold; padding: 4px 8px; vertical-align: middle;">${item.amount?.toFixed(2) || ''}</td>
        </tr>
      `
    }).join('')

    const emptyRows = Array.from({ length: Math.max(0, 18 - (items?.length || 0)) }).map(() => `
      <tr style="height: 32px;">
    ...
    const fullHTML = `
      ...
      <head>
        ...
        <style>
          ...
          .watermark-ms {
            position: absolute; top: 45%; left: 50%; transform: translate(-50%, -50%); pointer-events: none; user-select: none; z-index: 0; opacity: 0.12; font-size: 320px; font-weight: 900; letter-spacing: 25px; font-family: "Playfair Display", serif; color: #c0c0c0;
          }
          .content-wrapper { position: relative; z-index: 10; display: flex; flex-direction: column; height: 100%; width: 100%; gap: 16px; }
        ...
        <div class="a4-page">
          ...
          <div class="content-wrapper">
            ...
            <div style="text-align: center;">
              <div style="font-size: 10px; color: #6b7280; font-weight: bold; text-transform: uppercase;">${!isKacchi ? 'Subject to Sangli Jurisdiction' : ''}</div>
              <div style="display: flex; align-items: start; margin: 4px 0;">
                <div style="flex: 1;"></div>
                <div style="flex: 1; text-align: center;">
                  <div style="background-color: #dc2626; color: white; padding: 4px 32px; border-radius: 2px; font-size: 11px; font-weight: 900; letter-spacing: 2px; display: inline-block; text-transform: uppercase;">
              ...
              <h1 class="red-text" style="font-size: 48px; font-weight: bold; margin: 4px 0; letter-spacing: -1px;">M S TRADING COMPANY</h1>
              <div style="font-size: 10px; font-weight: bold; color: #374151; text-transform: uppercase; letter-spacing: 1px;">KUPWAD MIDC NEAR NAV KRISHNA VALLEY, PLOT NO L-52</div>
              ${!isKacchi ? `<div style="font-size: 11px; font-weight: bold; color: #1f2937; margin-top: 4px;">GST IN : ${companyGst}</div>` : ''}
            ...
            <div style="display: grid; grid-template-cols: 1fr 1fr 1fr; align-items: center; padding: 8px 0;">
              ...
              <div style="text-align: center;">
                ...
                <div class="red-text" style="font-size: 24px; font-weight: 900;">${formattedBillNo}</div>
              </div>
              ...
              <table style="width: 100%; border-collapse: collapse; font-size: 13px; table-layout: fixed;">
                <thead style="background-color: #f9fafb;">
                  <tr>
                    ...
                    <th style="border: 1px solid #9ca3af; padding: 8px; text-align: center; font-weight: 900; text-transform: uppercase; font-size: 11px; width: 96px;">Qty. Bags</th>
                    <th style="border: 1px solid #9ca3af; padding: 8px; text-align: center; font-weight: 900; text-transform: uppercase; font-size: 11px; width: 112px;">Weight kg</th>
                    <th style="border: 1px solid #9ca3af; padding: 8px; text-align: center; font-weight: 900; text-transform: uppercase; font-size: 11px; width: 96px;">Rate</th>
                    <th style="border: 1px solid #9ca3af; padding: 8px; text-align: right; font-weight: 900; text-transform: uppercase; font-size: 11px; width: 128px;">Amount</th>
                  </tr>
                </thead>
            ...
            <div style="margin-top: 12px; border-top: 3px solid black; padding-top: 8px;">
              <div style="display: grid; grid-template-cols: 1.2fr 1fr; gap: 20px;">
                <div><div style="font-weight: bold; font-size: 10px; color: #6b7280; text-transform: uppercase; margin-bottom: 4px;">Rs. in Words:</div><div style="font-size: 11px; font-weight: bold; border-bottom: 1px solid #e5e7eb; padding-bottom: 8px;">${totalInWords}</div></div>
                <div style="text-align: right;">
                  <div style="display: flex; justify-content: space-between; font-size: 14px;"><span class="bold gray-text">SUB TOTAL</span><span class="bold">₹ ${(bill.total_amount || 0).toFixed(2)}</span></div>
                  ${!isKacchi && bill.is_gst_enabled && (bill.gst_total || 0) > 0 ? `<div style="margin-top: 4px; border-top: 1px solid #f3f4f6; padding-top: 4px;">
                    <div style="display: flex; justify-content: space-between; font-size: 12px; color: #4b5563;"><span>GST TOTAL</span><span class="bold black-text">₹ ${bill.gst_total.toFixed(2)}</span></div>
                  </div>` : ''}
                  ${bill.balance > 0 ? `<div style="display: flex; justify-content: space-between; margin-top: 4px; font-size: 14px;"><span class="bold gray-text uppercase">BALANCE</span><span class="bold" style="color: #ea580c;">₹ ${bill.balance.toFixed(2)}</span></div>` : ''}
                  <div style="border-top: 3px solid black; margin-top: 8px; padding-top: 4px; display: flex; justify-content: space-between; align-items: center;">
                    <span style="font-size: 20px; font-weight: 900; font-style: italic;">TOTAL</span><span style="font-size: 24px; font-weight: 900;">₹ ${grandTotal.toFixed(2)}</span>
                  </div>
                </div>
              </div>
              <!-- Signatory Section -->
              <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-top: 24px;">
                <div style="text-align: left; width: 50%;">
                  ${(!isKacchi && bill.bank_name) ? `<div class="red-text bold" style="font-size: 11px; margin-bottom: 4px;">BANK DETAILS:</div>
                ...
                <div style="text-align: right;">
                  <div class="red-text bold" style="font-size: 11px; margin-bottom: 30px; text-transform: uppercase;">For M S TRADING COMPANY</div>
                  <div style="font-size: 10px; font-weight: 500; width: 180px; border-top: 1px solid #9ca3af; text-align: center; padding-top: 4px; color: #4b5563;">Auth. Signatory</div>
                </div>
              </div>
            </div>
          </div>
